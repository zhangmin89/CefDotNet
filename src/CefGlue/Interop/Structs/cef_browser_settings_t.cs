...
namespace Xilium.CefGlue.Interop
{
    ...
    internal unsafe struct cef_browser_settings_t
    {
        ...
        public static void Free(cef_browser_settings_t* ptr)
        {
            ...
            Clear(&ptr->standard_font_family);
            Clear(&ptr->fixed_font_family);
            Clear(&ptr->serif_font_family);
            Clear(&ptr->sans_serif_font_family);
            Clear(&ptr->cursive_font_family);
            Clear(&ptr->fantasy_font_family);
            Clear(&ptr->default_encoding);
            Marshal.FreeHGlobal((IntPtr)ptr);
        }

        private static void Clear(cef_string_t* value)
        {
            if (value->_dtor != IntPtr.Zero)
            {
                libcef.string_clear(value);
            }
        }
        #endregion
    }
