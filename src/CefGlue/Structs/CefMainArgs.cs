namespace Xilium.CefGlue
{
    ...
    public sealed unsafe class CefMainArgs
    {
        ...
        private cef_main_args_t_posix* ToNativePosix()
        {
            ...
            if (_args != null)
            {
                _argcArgvBlock = MarshallArgcArgvBlock(_args);
                ptr->argv = _argcArgvBlock;
        ...
        private void FreeArgcArgvBlock()
        {
            if (_argcArgvBlock == IntPtr.Zero) return;

            Marshal.FreeHGlobal(_argcArgvBlock);
            _argcArgvBlock = IntPtr.Zero;
        }

        internal void Free(cef_main_args_t* ptr)
        {
            switch (CefRuntime.Platform)
            {
                case CefRuntimePlatform.Windows:
